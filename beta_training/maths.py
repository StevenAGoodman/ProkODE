...
# def transcription_rate(gene, protein_amnts, gene_key, N_rnap, Kd_rnap, gene_info_dict, genome_len, temperature, feature_id):
...
#         P = N_tf / (N_tf + score_to_K(tf_info["score"], temperature))
#         beta = tf_info["beta"]
...
def beta_curveFit_func(feature_id):
    ...
    if feature_id == "@":
        def beta_function(P_arr, *beta_arr):
            ...
            return np.nan_to_num(np.array(P_arr)) @ np.nan_to_num(np.array(beta_arr)).T
        
    elif feature_id == "A":
        def beta_function(P_arr, *beta_arr):
            assert len(P_arr[0]) == len(beta_arr)
            P_arr = np.nan_to_num(np.array(P_arr), nan=1e-100)
            beta_arr = np.nan_to_num(np.array(beta_arr), nan=1e-100)
            return np.log10(np.where(P_arr == 0, 1e-100, P_arr)) @ np.log10(np.where(beta_arr == 0, 1e-100, beta_arr)).T
        
    elif feature_id == "B":
        def beta_function(P_arr, *beta_arr):
            assert len(P_arr[0]) == len(beta_arr)
            beta_arr = np.nan_to_num(np.array(beta_arr), nan=1e-100)
            return np.nan_to_num(np.array(P_arr)) @ np.log10(np.where(beta_arr == 0, 1e-100, beta_arr)).T
...
def translation_rate(gene, gene_mrna_amount, protein_amnts, N_ribo, gene_info_dict, R_max_trans, Kd_ribo_mrna, temperature, feature_id):
    ...
    if feature_id == "@":
        ...
        m_ribo = (2500 * 1000000) / 1000 # g / mol -> kg
        ...
        translation_rate_raw = z * gene_mrna_amount * N_ribo * 1 / Kd_ribo_mrna
        return translation_rate_raw * gene_mrna_amount
...
def RNA_decay_rate(prev_gene_mRNA, protein_amnts, gene_info_dict, gene_key, temperature, growth_fid, binary_feature_arr):
    ...
    if binary_feature_arr[3] == 1:
        ...
        for degrad_prot, dp_info in decay_info.items():
            ...
            m_dp = dp_info["mass"] # (110 * dp_info["aa length"]) / 1000 # g / mol -> kg
            m_mRNA = (320 * gene_info_dict["mRNA length"] + 159) / 1000 # g / mol -> kg
            r_dp = 0.4e-8 # m
            r_mRNA = 1e-9 * (2 + 0.3 * gene_info_dict["mRNA length"]) / 2 # m
            z = 10e3 * (6.02214076e23) * np.pi * (r_dp + r_mRNA)**2 * np.sqrt( (8 * 1.987204259e-3 * temperature) / (np.pi * m_dp * m_mRNA / (m_dp + m_mRNA)) )
            translation_rate_raw = z * prev_gene_mRNA * N_dp * score_to_K(-delta_G, temperature)
